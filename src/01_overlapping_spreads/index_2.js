...
exports.overlappingSpreads = function (data) {
    const neighbouringObj = { memory: {}, sum: 0 }

    if (Array.isArray(data)) {
        data.forEach((value) => {
            const neighbours = [value-1, value+1]
            neighbouringObj.memory[neighbours[0]] ? neighbouringObj.sum += neighbours[0] : neighbouringObj.memory[neighbours[0]] = true;
            neighbouringObj.memory[neighbours[1]] ? neighbouringObj.sum += neighbours[1] : neighbouringObj.memory[neighbours[1]] = true;
        })
    }

    return neighbouringObj.sum
}
